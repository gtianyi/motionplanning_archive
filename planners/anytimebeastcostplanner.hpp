...
namespace control {
//use beast to find the first solution than do puring and sst
class AnytimeBeastCostPlanner : public ompl::control::RRT {
 protected:
  class Witness;

 public:
  /** \brief Constructor */
  AnytimeBeastCostPlanner(const SpaceInformationPtr &si,
                       const FileMap &params) :
      ompl::control::RRT(si), params(params) {
    setName("AnytimeBeastCostPlanner");
    propagationStepSize = siC_->getPropagationStepSize();
    Planner::declareParam<bool>("intermediate_states",
                                this,
                                &AnytimeBeastCostPlanner::
                                setIntermediateStates,
                                &AnytimeBeastCostPlanner::
                                getIntermediateStates);

    //Obviously this isn't really a parameter
    //but I have no idea how else to get it into the output file
    //through the benchmarker
    Planner::declareParam<double>("samplerinitializationtime",
                                  this,
                                  &AnytimeBeastCostPlanner::
                                  ignoreSetterDouble,
                                  &AnytimeBeastCostPlanner::
                                  getSamplerInitializationTime);
  }

  virtual ~AnytimeBeastCostPlanner() {
    freeMemory();
  }
  ...
  void setIntermediateStates(bool add) {
    addIntermediateStates_ = add;
  }
  ...
  virtual base::PlannerStatus solve(const base::
    ...
    optimizationObjective->setCostThreshold(
        optimizationObjective->infiniteCost());

    if(sstPruningModule != nullptr) {}
    else if(params.stringVal("SSTStyle").compare("None") == 0) {
      sstPruningModule = new NoSSTPruningModule<MotionWithCost,
                                                Motion>();
    } else if(params.stringVal("SSTStyle").compare("SST") == 0) {
      sstPruningModule =
          new SSTPruningModule
          <MotionWithCost,Motion>(this,
                                  siC_,
                                  optimizationObjective,
                                  params.doubleVal("SelectionRadius"),
                                  params.doubleVal("PruningRadius"));
    } else if(params.stringVal("SSTStyle").compare("SSTStar") == 0) {
      sstPruningModule =
          new SSTStarPruningModule
          <MotionWithCost, Motion>(this,
                                   siC_,
                                   optimizationObjective,
                                   params.doubleVal("SelectionRadius"),
                                   ...
                                   siC_->getStateSpace()
                                   ->getDimension(),
                                   siC_->getControlSpace()
                                   ->getDimension());
    } else {
      throw ompl::Exception("Unrecognized SSTStyle: %s",
                            params.stringVal("SSTStyle"));
    }

    if(costPruningModule != nullptr) {}
    else if(params.stringVal("CostPruningStyle").compare("None") == 0) {
      costPruningModule = new NoCostPruningModule
          <MotionWithCost>(optimizationObjective);
    } else if(params.stringVal("CostPruningStyle").compare("G") == 0) {
      costPruningModule = new GCostPruningModule
          <MotionWithCost>(optimizationObjective);
    } else if(params.stringVal("CostPruningStyle").compare("F") == 0) {
      costPruningModule = new FCostPruningModule
          <MotionWithCost>(optimizationObjective, goal);
    } else {
      throw ompl::Exception("Unrecognized CostPruningStyle: %s",
                            params.stringVal("CostPruningStyle"));
    }
    ...
    if(!newsampler) {
      ...
      newsampler = new ompl::
          base::
          refactored::
          AnytimeBeastSampler((ompl::base::SpaceInformation *)siC_,
    ...
    OMPL_INFORM("%s: Starting planning with %u states "
    ...
    while(ptc == false) {
      ...
      //use beast to find the first solution,
      //then just do sst and cost purnning
      if (!firstsolved){
      ...
      unsigned int cd = controlSampler_->sampleTo(rctrl,
                                                  nmotion->control,
                                                  nmotion->state,
                                                  rmotion->state);
      ...
      if(addIntermediateStates_) {
        ...
        cd = siC_->propagateWhileValid(nmotion->state,
                                       rctrl,
                                       cd,
                                       pstates,
                                       true);
        ...
        if(cd >= siC_->getMinControlDuration()) {
          ...
          for(; p < pstates.size(); ++p) {
            ...
            if(solved &&
               optimizationObjective->isSatisfied(motion->g)) {
              globalParameters.solutionStream.addSolution(motion->g,
                                                          start);
          ...
          // post process the branch
          //just added for SST* like pruning from leaf to root
          for (auto addedMotionsIterator = addedMotions.rbegin();
               addedMotionsIterator != addedMotions.rend();
               ++addedMotionsIterator) {
            MotionWithCost *motion = *addedMotionsIterator;
            auto prunedAndWasWitness = sstPruningModule->
                shouldPrune(motion);
            ...
            if(prunedAndWasWitness.second ||
               prunedAndWasWitness.first == nullptr) {
              motion->isInDatastructures = true;
              newsampler->
                  reached(motion->parent->state,
                          ((MotionWithCost*)motion->parent)->g.value(),
        ...
        if(cd >= siC_->getMinControlDuration()) {
          ...
          ompl::base::Cost h = optimizationObjective->
              costToGo(motion->state, goal);
          if(!costPruningModule->shouldPrune(motion)) {
            double dist = 0.0;
            ...
            auto prunedAndWasWitness =
                sstPruningModule->shouldPrune(motion);
            if(prunedAndWasWitness.second) {
              nn_->remove(prunedAndWasWitness.first);
              newsampler->remove(prunedAndWasWitness.first->state,
                                 prunedAndWasWitness.first->g.value());
              sstPruningModule->cleanupTree(prunedAndWasWitness.first);
            }
            if(prunedAndWasWitness.second ||
               prunedAndWasWitness.first == nullptr){
    ...
    return ompl::base::PlannerStatus(firstsolved, false);
  }
 ...
 protected:
  ...
  class MotionWithCost : public Motion {
   public:
    ...
    void updateGValue(double propagationStepSize) {
      g = ompl::base::Cost(((MotionWithCost*)parent)->g.value() +
                           propagationStepSize * steps);
    }
  ...
  // ompl::base::AnytimeBeastSampler *newsampler = NULL;
  	...
  	/** \brief State sampler */
  ompl::base::StateSamplerPtr uniformsampler_;
